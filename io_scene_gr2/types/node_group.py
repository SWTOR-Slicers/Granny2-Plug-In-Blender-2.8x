...
def convert_hsl_to_rgb():
    ...
    if blender_version < 5:
        ...
        combine_hsv = node_tree.nodes.new(type='ShaderNodeCombineColor')
    ...
    node_tree.links.new(node_reroute_3.outputs[0], node_reroute_4.inputs[0])
    node_tree.links.new(group_input_5.outputs['H'], combine_hsv.inputs[0])
    node_tree.links.new(math_10.outputs['Value'], combine_hsv.inputs[1])
    node_tree.links.new(node_reroute_4.outputs[0], combine_hsv.inputs[2])
...
def set_alpha_mode():
    ...
    # This node group's code was generated by a different nodes-to-code extension:
    # https://extensions.blender.org/add-ons/node-to-python/
    # and retouched, hence some differences in style
    
    # Check if node tree already exists
    if 'SetAlphaMode' in bpy.data.node_groups:
        return bpy.data.node_groups['SetAlphaMode']

    # Make new node tree and add input/output sockets
    node_tree = bpy.data.node_groups.new(name = "SetAlphaMode", type = 'ShaderNodeTree')
    if blender_version < 4.0:
        alpha_socket = node_tree.outputs.new(name="Alpha", type='NodeSocketFloat')
        alpha_socket_1 = node_tree.inputs.new(name="Alpha", type='NodeSocketFloat')
        isalphamodeblend_socket = node_tree.inputs.new(name="IsAlphaModeBlend", type='NodeSocketBool')
        isalphamodetest_socket = node_tree.inputs.new(name="IsAlphaModeTest", type='NodeSocketBool')
        alphatestvalue_socket = node_tree.inputs.new(name="AlphaTestValue", type='NodeSocketFloat')
        invert_socket = node_tree.inputs.new(name="Invert", type='NodeSocketBool')
    else:
        alpha_socket = node_tree.interface.new_socket(name="Alpha", in_out='OUTPUT', socket_type='NodeSocketFloat')
        alpha_socket_1 = node_tree.interface.new_socket(name="Alpha", in_out='INPUT', socket_type='NodeSocketFloat')
        isalphamodeblend_socket = node_tree.interface.new_socket(name="IsAlphaModeBlend", in_out='INPUT', socket_type='NodeSocketBool')
        isalphamodetest_socket = node_tree.interface.new_socket(name="IsAlphaModeTest", in_out='INPUT', socket_type='NodeSocketBool')
        alphatestvalue_socket = node_tree.interface.new_socket(name="AlphaTestValue", in_out='INPUT', socket_type='NodeSocketFloat')
        invert_socket = node_tree.interface.new_socket(name="Invert", in_out='INPUT', socket_type='NodeSocketBool')
    ...
    # Input Socket Alpha
    alpha_socket_1.default_value = 0.0
    alpha_socket_1.min_value = 0.0
    alpha_socket_1.max_value = 1.0

    # Socket IsAlphaModeBlend
    isalphamodeblend_socket.default_value = False
    isalphamodeblend_socket.description = "For Alpha = None, uncheck both checkboxes"

    # Socket IsAlphaModeTest
    isalphamodetest_socket.default_value = True
    isalphamodetest_socket.description = "For Alpha = None, uncheck both checkboxes"

    # Socket AlphaTestValue
    alphatestvalue_socket.default_value = 0.5
    alphatestvalue_socket.min_value = 0.0
    alphatestvalue_socket.max_value = 1.0

    # Socket Invert
    invert_socket.default_value = False
    invert_socket.description = "Set to True for modernized characters' head SkinB materials"
    ...
    # Set locations
    node_tree.nodes["Group Output"].location = (727.2438354492188, 108.56954956054688)
    node_tree.nodes["Group Input"].location = (-585.5767822265625, 32.59794616699219)
    node_tree.nodes["Math.004"].location = (71.92079162597656, -62.25663757324219)
    node_tree.nodes["Math"].location = (527.8878173828125, 108.2802734375)
    node_tree.nodes["Math.001"].location = (291.1736755371094, -94.93627166748047)
    node_tree.nodes["Math.002"].location = (76.37089538574219, 217.50466918945312)
    node_tree.nodes["Invert Color"].location = (-252.53656005859375, 27.780345916748047)
    node_tree.nodes["Reroute.001"].location = (-308.14532470703125, -193.05908203125)
    node_tree.nodes["Reroute.002"].location = (-262.6187744140625, 87.13911437988281)
    node_tree.nodes["Reroute"].location = (-202.2725830078125, -225.06643676757812)

    # Set dimensions
    node_tree.nodes["Group Output"].width  = 140.0
    node_tree.nodes["Group Output"].height = 100.0

    node_tree.nodes["Group Input"].width  = 140.0
    node_tree.nodes["Group Input"].height = 100.0

    node_tree.nodes["Math.004"].width  = 140.0
    node_tree.nodes["Math.004"].height = 100.0

    node_tree.nodes["Math"].width  = 140.0
    node_tree.nodes["Math"].height = 100.0

    node_tree.nodes["Math.001"].width  = 140.0
    node_tree.nodes["Math.001"].height = 100.0

    node_tree.nodes["Math.002"].width  = 140.0
    node_tree.nodes["Math.002"].height = 100.0

    node_tree.nodes["Invert Color"].width  = 140.0
    node_tree.nodes["Invert Color"].height = 100.0

    node_tree.nodes["Reroute.001"].width  = 16.0
    node_tree.nodes["Reroute.001"].height = 100.0

    node_tree.nodes["Reroute.002"].width  = 16.0
    node_tree.nodes["Reroute.002"].height = 100.0

    node_tree.nodes["Reroute"].width  = 16.0
    node_tree.nodes["Reroute"].height = 100.0
    ...
    node_tree.links.new(
        node_tree.nodes["Reroute.001"].outputs[0],
        node_tree.nodes["Math.004"].inputs[1]
    )
    # math_001.Value -> math.Value
    node_tree.links.new(
        node_tree.nodes["Math.001"].outputs[0],
        node_tree.nodes["Math"].inputs[1]
    )
    # reroute_002.Output -> math_002.Value
    node_tree.links.new(
        node_tree.nodes["Reroute.002"].outputs[0],
        node_tree.nodes["Math.002"].inputs[1]
    )
    # math_002.Value -> math.Value
    node_tree.links.new(
        node_tree.nodes["Math.002"].outputs[0],
        node_tree.nodes["Math"].inputs[0]
    )
    # math.Value -> group_output.Alpha
    node_tree.links.new(
        node_tree.nodes["Math"].outputs[0],
        node_tree.nodes["Group Output"].inputs[0]
    )
    # group_input.Alpha -> invert_color.Color
    node_tree.links.new(
        node_tree.nodes["Group Input"].outputs[0],
        node_tree.nodes["Invert Color"].inputs[1]
    )
    # group_input.Invert -> invert_color.Fac
    node_tree.links.new(
        node_tree.nodes["Group Input"].outputs[4],
        node_tree.nodes["Invert Color"].inputs[0]
    )
    # invert_color.Color -> math_004.Value
    node_tree.links.new(
        node_tree.nodes["Invert Color"].outputs[0],
        node_tree.nodes["Math.004"].inputs[0]
    )
    # invert_color.Color -> math_002.Value
    node_tree.links.new(
        node_tree.nodes["Invert Color"].outputs[0],
        node_tree.nodes["Math.002"].inputs[0]
    )
    # group_input.AlphaTestValue -> reroute_001.Input
    node_tree.links.new(
        node_tree.nodes["Group Input"].outputs[3],
        node_tree.nodes["Reroute.001"].inputs[0]
    )
    # group_input.IsAlphaModeBlend -> reroute_002.Input
    node_tree.links.new(
        node_tree.nodes["Group Input"].outputs[1],
        node_tree.nodes["Reroute.002"].inputs[0]
    )
    # group_input.IsAlphaModeTest -> reroute.Input
    node_tree.links.new(
        node_tree.nodes["Group Input"].outputs[2],
        node_tree.nodes["Reroute"].inputs[0]
    )
    # math_004.Value -> math_001.Value
    node_tree.links.new(
        node_tree.nodes["Math.004"].outputs[0],
        node_tree.nodes["Math.001"].inputs[0]
    )
    # reroute.Output -> math_001.Value
    node_tree.links.new(
        node_tree.nodes["Reroute"].outputs[0],
        node_tree.nodes["Math.001"].inputs[1]
    )
